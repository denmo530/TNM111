const { BuildSharp } = require("@mui/icons-material");
...
function focusPlusContext(data) {
  ...
  brush = d3.brushX().extent([0, 0], [width, height2]).on(brushed());

  //Setting scale parameters
  var maxDate = d3.max(data.features, function (d) {
    return parseDate(d.properties.Date);
  });
  var minDate = d3.min(data.features, function (d) {
    return parseDate(d.properties.Date);
  });
  var maxMag = d3.max(data.features, function (d) {
    return d.properties.EQ_PRIMARY;
  });
  var minMag = d3.min(data.features, function (d) {
    return d.properties.EQ_PRIMARY;
  });
  ...
  function mouseOver(selected_dots) {
    selected_dots.on("mouseover", function (d) {
      /**
  ...
  function mouseOut(selected_dots) {
    selected_dots.on("mouseout", function () {
  ...
  function brushed() {
    ...
    if (d3.event.type == "end") {
      ...
      d3.selectAll(".dot").each(function (d, i) {
        if (
          parseDate(d.properties.Date) >= xScale.domain()[0] &&
